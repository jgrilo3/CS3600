...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        #add bias
        inActs.insert(0, 1.0)
        result = self.sigmoid(self.getWeightedSum(inActs))
        #remove bias
        inActs.pop(0)
        return result
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        #add bias
        inActs.insert(0, 1.0)
        result = self.sigmoidDeriv(self.getWeightedSum(inActs))
        #remove bias
        inActs.pop(0)
        return result
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """YOUR CODE"""
        inActs.insert(0, 1.0)
        for act in range(len(inActs)):
            val = alpha * delta * inActs[act]
            self.weights[act] += val
            totalModification += abs(val)
        inActs.pop(0)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            addOn = []
            for perc in layer:
                addOn.append(perc.sigmoidActivation(prevAct))
            result.append(addOn)
            prevAct = addOn
        return result
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    total = 0
                    for num in range(len(nextLayer)):
                        total += deltas[0][num] * nextLayer[num].weights[neuronNum + 1]
                    delta = gPrime * total
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for inp, out in examplesTest:
        ff = nnet.feedForward(inp)[-1]
        rounds = []
        for item in ff:
            rounds.append(round(item))
        if rounds == out:
            testCorrect += 1
        else:
            testError += 1
    ...
    testAccuracy= testCorrect / (testError + testCorrect + 0.0)
