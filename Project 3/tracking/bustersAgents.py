...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distribution in livingGhostPositionDistributions:
            pos = distribution.argMax()
            dist = self.distancer.getDistance(pacmanPosition, pos)
            # check to see if lower distance exists for this position
            if dist < minDist:
                minDist = dist
                minPos = pos

        # find action that will take you the closest to the min position
        minDistNew = float("inf")
        idealAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            successorDist = self.distancer.getDistance(minPos, successorPosition)
            # check to see which action results in lowest distance for this min position
            if successorDist < minDistNew:
                minDistNew = successorDist
                idealAction = action
