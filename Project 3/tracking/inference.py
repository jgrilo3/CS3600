...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # check edge case which is that obs is NULL
        if observation is None:
            # set all legal (non jail) positions to 0
            for potential in self.legalPositions:
                self.beliefs[potential] = 0
            # normalize list
            self.beliefs.normalize()
            return
        else:
            # iterate through each potential position and update beliefs based upon the probabilities at these positions
            for potential in self.legalPositions:
                distance = util.manhattanDistance(potential, pos)
                # beliefs[potential] (P(noisyDistance)) = P(noisyDistance | trueDistance) * P(trueDistance)
                update[potential] = busters.getObservationProbability(observation, distance) * self.beliefs[potential]
        self.beliefs = update
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        newPosDist is a DiscreteDistribution object, where for each position p in self.legalPositions, newPosDist[p] is the probability
        that the ghost is at position p at time t + 1, given that the ghost is at position oldPos at time t

        """
        "*** YOUR CODE HERE ***"
        update = util.Counter()
        # Iterate through each position available
        for previous in self.allPositions:
            # generate an action distribution for each of these positions
            newPosDist = self.getPositionDistribution(gameState, previous)
            # iterate through distribution
            for position, prob in newPosDist.items():
                # update belief map based on data
                update[position] = update[position] + self.beliefs[previous] * prob
        self.beliefs = update
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        # iterate through the total # of particles
        num = 0
        while num < self.numParticles:
            # iterate through all legal positions
            for pos in self.legalPositions:
                # append each position and increment num
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if ghost caught, update particle list
        partList = util.Counter()
        if observation == None:
            for i in range(self.numParticles):
                self.particles[i] = self.getJailPosition()

        else:
            # keep track of how many times each particle appears
            for particle in self.particles:
                partList[particle] += 1

            #iterate through particle list, multiplying dict value by probability and obs prob
            for pos, prob in partList.items():
                dist = util.manhattanDistance(pos, gameState.getPacmanPosition())
                partList[pos] = prob * busters.getObservationProbability(observation, dist)

        if partList.totalCount():
            # if nonzero values in partList, add semples of partList into particles
            self.particles = [util.sample(partList) for i in range(self.numParticles)]
        else:
            # edge case: reinitialize gameState
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        partList = []
        # gather the probability distributions for each particle and add into self.particles
        for particle in self.particles:
            newPosDist = self.getPositionDistribution(gameState, particle)
            partList.append(newPosDist.sample())
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # iterate through the particles and increment dist at respective particle indecies
        dist = util.Counter()
        for part in self.particles:  
            dist[part] = dist[part] + 1
        dist.normalize()
        return dist
